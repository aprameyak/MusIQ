...
router.get('/', authMiddleware, async (req: AuthRequest, res, next) => {
  try {
    ...
    const userResult = await pool.query(
      `SELECT 
          u.id, u.email, u.username, u.email_verified, u.mfa_enabled, u.role, u.oauth_provider, u.oauth_id, u.last_login_at, u.created_at, u.updated_at, u.profile_picture_url,
...
router.get('/taste', authMiddleware, async (req: AuthRequest, res, next) => {
  try {
    if (!req.userId) {
      throw new CustomError('Unauthorized', 401);
    }
    ...
    const tasteScore = Math.round(
      (parseFloat(stats.avg_rating) || 0) * 10 + Math.min(parseInt(stats.total_ratings) / 10, 50)
    );
    ...
    const influence = Math.round(
      parseInt(stats.total_ratings) * (parseFloat(stats.avg_rating) || 0) * 100
    );
...
router.put(
  ...
  async (req: AuthRequest, res, next) => {
    try {
      ...
      const userResult = await pool.query(
        `SELECT id, email, username, first_name, last_name, profile_picture_url, email_verified, mfa_enabled, role, oauth_provider, oauth_id, last_login_at, created_at, updated_at
...
router.get('/:userId', authMiddleware, async (req: AuthRequest, res, next) => {
  try {
    if (!req.userId) {
      throw new CustomError('Unauthorized', 401);
    }
    ...
    const userResult = await pool.query(
      `SELECT 
          u.id, u.username, u.email, u.created_at, u.profile_picture_url,
          (SELECT COUNT(*) FROM friendships WHERE user_id = u.id AND status = 'accepted') as following_count,
          (SELECT COUNT(*) FROM friendships WHERE friend_id = u.id AND status = 'accepted') as followers_count,
          (SELECT COUNT(*) FROM posts WHERE user_id = u.id) as posts_count,
...
router.post(
  ...
  async (req: AuthRequest, res, next) => {
    try {
      ...
      if (!req.file) {
        throw new CustomError('No picture provided', 400);
      }

      const user = await authService.getUserById(req.userId);
      if (user?.profile_picture_url) {
        await blobService.deleteProfilePicture(user.profile_picture_url);
      }

      const imageUrl = await blobService.uploadProfilePicture(
        req.userId,
        req.file.buffer,
        req.file.mimetype
      );

      await authService.updateProfile(req.userId, undefined, undefined, undefined, imageUrl);
...
router.delete('/picture', authMiddleware, async (req: AuthRequest, res, next) => {
  try {
    if (!req.userId) {
      throw new CustomError('Unauthorized', 401);
    }
    ...
    await authService.updateProfile(req.userId, undefined, undefined, undefined, null as any);
